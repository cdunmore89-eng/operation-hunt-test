...
(function connectDivisionPresentation() {
    ...
    if (
        ...
        console.error(
            "Operation Hunt division presentation could not initialize."
        );
        return;
    }
    ...
    renderGameMode = function renderAllArithmeticModes() {
        ...
        if (operation === OPERATIONS.SUBTRACTION) {
            ...
            const divisorCount = termCount - 1;

            elements.gameInstructions.textContent = practice
                ? `Select the dividend first, then ${divisorCount} divisor${divisorCount === 1 ? "" : "s"} in any order. Match the whole-number quotient and keep practicing.`
                : maxOut
                    ? `Select the dividend first, then ${divisorCount} divisor${divisorCount === 1 ? "" : "s"}. Match as many whole-number quotients as possible before time expires.`
                    : `Select the dividend first, then ${divisorCount} divisor${divisorCount === 1 ? "" : "s"} in any order. Strike the matching whole-number quotient on the radar.`;
        } else {
            elements.gameInstructions.textContent = practice
